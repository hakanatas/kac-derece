...
async function exportFormat(browser, format, ffmpeg) {
  ...
  const base = `kac-derece_${format}_${CAPS}`;
  ...
  const enc = spawn(ffmpeg, ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(FPS), '-c:v', FRAMES === 'jpeg' ? 'mjpeg' : 'png', '-i', '-',
    ...
    '-metadata', 'title=Kaç Derece? — Açıları Ölçelim', mp4], { stdio: ['pipe', 'inherit', 'inherit'] });
...
console.log(`Kaç Derece? → MP4  (${FPS} fps, captions: ${CAPS}, ${WORKERS} workers)`);
